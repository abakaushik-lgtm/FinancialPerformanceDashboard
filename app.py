...
# Flexible Filter Mask: If any selection is empty, treat as "All Selected"
mask_year = df_raw['Year'].isin(selected_years) if selected_years else pd.Series(True, index=df_raw.index)
mask_quarter = df_raw['Quarter'].isin(selected_quarters) if selected_quarters else pd.Series(True, index=df_raw.index)
mask_region = df_raw['Region'].isin(selected_regions) if selected_regions else pd.Series(True, index=df_raw.index)
mask_category = df_raw['Product Category'].isin(selected_categories) if selected_categories else pd.Series(True, index=df_raw.index)
mask_bu = df_raw['Business Unit'].isin(selected_bus) if selected_bus else pd.Series(True, index=df_raw.index)
mask_dept = df_raw['Department'].isin(selected_depts) if selected_depts else pd.Series(True, index=df_raw.index)

mask = mask_year & mask_quarter & mask_region & mask_category & mask_bu & mask_dept
df_filtered = df_raw[mask].copy()

# Guaranteed Fallback: If filtered dataframe ever evaluates to empty, default to full dataset
